...
<script type="text/javascript">
		...
		$("#upload")
				.click(
						function() {
							...
							var url = "";
							if (value == 1) {
								url = "http://192.168.1.106:8080/marc/uploadMarc";
							} else {
								url = "http://192.168.1.106:8080/marc/uploadExcel"
							}
							$.ajaxFileUpload({
	...
	function searchSchool() {
		...
		$.ajax({
			url : "${pageContext.request.contextPath}/comp/marc/searchSchool",
			type : "POST",
			dataType : "json",
			data : {
	...
	/***获取馆藏地点和文献流通类型**/
	function chooseCollectionPlaceAndType() {
		var schoolName = $("#schoolName").val();
		$
				.ajax({
					url : "${pageContext.request.contextPath}/comp/marc/getPlaceAndType",
					type : "POST",
					dataType : "json",
					data : {
						"schoolName" : schoolName
	...
	<form action="${pageContext.request.contextPath}/comp/marc/upload"
		...
		<div class="main">
			<div class="path">
				...
				<div class="path_module">当前位置：采编管理 → MARC导入</div>
				<div></div>
					...
					<div class="row_half_right">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td align="left"><input name="" value="" id="schoolInput"><select
									onchange="chooseCollectionPlaceAndType();" name="schoolName"
									id="schoolName">
										<option value="0">--请选择--</option>

								</select><input class="input_btn" type="button" value="查询"
									onclick="searchSchool();"><span style="color: red"
									id="marcInfo">*</span></td>
							</tr>
							<tr>
								<th align="right">馆藏地点：</th>
								<td align="left"><select name="place" id="place">
