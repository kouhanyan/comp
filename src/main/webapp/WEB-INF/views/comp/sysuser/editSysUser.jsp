...
<body>
	<form action="${pageContext.request.contextPath}/comp/sysUser/edit"
		method="post" id="formId">
		<input type="hidden" name="id" value="${sysUser.id}">
		<div class="main">
			<div class="path">
				<div class="path_left"></div>
				<div class="path_module">当前位置：系统管理 → 系统用户管理 → 系统用户修改</div>
				<div ></div>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td align="left"><input name="name" type="text"
									value="${sysUser.name}" id="name"/><span style="color: red" id="nameInfo">*</span></td>
							</tr>
							<tr>
								<th align="right">登录名称：</th>
								<td align="left"><input name="userName" type="text"
									value="${sysUser.userName}" id="userName">
									<span style="color: red" id="userInfo">*</span></td>
							</tr>
							<tr>
								<th align="right">登录密码：</th>
								<td align="left"><input name="password" type="password"
									value="" id="password" /> <span
									style="color:gray">留空不修改</span></td>
							</tr>
							<tr>
								<th align="right">所属学校：</th>
								<td align="left"><select name="school.id" id="schoolId" onchange="getRoles(this,'${pageContext.request.contextPath}/comp/sysUser/getRoles')">
								<option value="0">--请选择--</option>
									...
									value="查询" /><span id="schoolNameSpan" style="color: red;display:none;" >数据加载中,请稍候</span></td>
							</tr>
							<tr>
								...
								<td colspan="3" align="left"><select name="sysRole.id"
									id="sysRoleId">
									<option value="0">--请选择--</option>
										<c:forEach items="${sysRoles}" var="i">
											<option value="${i.id}"
												<c:if test="${sysUser.sysRole.id==i.id}">selected="selected"</c:if>>${i.name}</option>
										</c:forEach>
