...
<form action="${pageContext.request.contextPath}/comp/sysRole/edit"
	method="post" id="formId"><input type="hidden" name="id"
	value="${sysRole.id}">
<div class="main">
<div class="path">
<div class="path_left"></div>
<div class="path_module">当前位置：系统管理 → 系统角色管理→ 系统角色修改</div>
<div></div>
<div class="path_right"><a href="javascript:history.go(1);"
	class="btn_next">向前</a> <a href="javascript:history.go(-1);"
	class="btn_back">返回</a></div>
</div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td align="left"><input name="name" type="text"
			value="${sysRole.name}" id="name" /><span style="color: red"
			id="nameInfo">*</span></td>
	</tr>
	<tr>
		...
		</select><span style="color: red" id="schoolInfo">*</span><input type="text"
			name="schoolNameSelect" /><input name="schoolNameInput"
			class="input_btn" type="button"
	...
	<tr>
		...
		<td colspan="3" align="left"><textarea name="roleDesc"
			id="textarea" cols="50" rows="3">${sysRole.roleDesc}</textarea></td>
	</tr>
	...
	<tr>
			...
			<c:forEach items="${sysResources}" var="i">
				<c:if test="${i.resourceLevel==1}">
					...
					<input type="checkbox" name="checks" value="${i.id}"
						onclick="click_f_menu(this);"
						<c:forEach items="${sysRoleResources}" var="srr">
													<c:if test="${srr.sysResource.id==i.id}">checked="checked"</c:if></c:forEach> />${i.name}
					<ul>
						<c:forEach items="${sysResources}" var="j">
							<c:if test="${i.id==j.parentId}">
								<li class="collapsable">
								<div class="hitarea collapsable-hitarea"></div>
								<input type="checkbox" name="checks" value="${j.id}" onclick="click_s_menu(this);"
									<c:forEach items="${sysRoleResources}" var="srr">
													<c:if test="${srr.sysResource.id==j.id}">checked="checked"</c:if></c:forEach> />${j.name}
								<ul>
									<c:forEach items="${sysResources}" var="k">
										<c:if test="${j.id==k.parentId}">
											<li class="collapsable">
											<div class="hitarea collapsable-hitarea"></div>
											<input type="checkbox" name="checks" value="${k.id}"
												onclick="click_t_menu(this);"
												<c:forEach items="${sysRoleResources}" var="srr">
													<c:if test="${srr.sysResource.id==k.id}">checked="checked"</c:if></c:forEach> />${k.name}
											</li>
										</c:if>
									</c:forEach>
								</ul>
		...
		<div class="btn"><input type="button" value="全/反选"
			onclick="checkIds()" /></div>
		</td>
	</tr>

	<tr>
		<th>&nbsp;</th>
		<td><input name="" class="input_btn" type="button" value="保存"
			onclick="judgeSubmit()" /> <input name="" class="input_btn"
			type="button" value="重置" onclick="formReset()" /> <input name=""
			class="input_btn" type="button" value="返回"
